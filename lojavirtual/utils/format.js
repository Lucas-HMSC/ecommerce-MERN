...
export const formatDataDeNascimento = (value) => {
  ...
  if (_data.length > 4)
    return (
      _data.slice(0, 2) + '/' + _data.slice(2, 4) + '/' + _data.slice(4, 8)
    );
  if (_data.length > 2) _data.slice(0, 2) + '/' + _data.slice(2, 4);
  return _data;
};
...
export const formatTelefone = (value) => {
  ...
  return _telefone.length > 2
    ? _telefone.slice(0, 2) + ' ' + _telefone.slice(2, 12)
    : _telefone;
};
