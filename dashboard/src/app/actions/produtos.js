...
export const salvarProduto = (produto, loja, cb) => {
  return function (dispatch) {
    axios
      .post(
        `${api}/${versao}/api/produtos?loja=${loja}`,
        {
          titulo: produto.nome,
...
export const getProduto = (id, loja) => {
  return function (dispatch) {
    axios
      .get(`${api}/${versao}/api/produtos/${id}?loja=${loja}`, getHeaders())
      .then((response) =>
        dispatch({
          type: GET_PRODUTO,
          payload: response.data,
...
export const updateProduto = (produto, id, loja, cb) => {
  return function (dispatch) {
    axios
      .put(
        `${api}/${versao}/api/produtos/${id}?loja=${loja}`,
        {
          titulo: produto.nome,
          descricao: produto.descricao,
          disponibilidade:
            produto.disponibilidade === 'disponivel' ? 'true' : 'false',
          categoria: produto.categoria,
          preco: produto.preco,
          promocao: produto.promocao,
          sku: produto.sku,
...
export const updateProdutoImagens = (data, id, loja, cb) => {
  return function (dispatch) {
    axios
      .put(
        `${api}/${versao}/api/produtos/images/${id}?loja=${loja}`,
        data,
        getHeaders(),
      )
      .then((response) => {
